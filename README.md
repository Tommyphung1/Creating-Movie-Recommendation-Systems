# Recommendation System Using Knn With Mean
...
The **second** way is to take the corresponding rank of the predicted values from the models and take **the average of the ratings from their sum**. The second way appears to be a better prediction than the previous models with a small difference in RSME. 
...
## Data Understanding
The dataset was compiled by the **Grouplens** research group and the source of that data comes from [MovieLens](https://movielens.org/). MovieLens is a movie recommendation service that has **9,742 movies** and **100,836 ratings** from **610 users**. The dataset was updated on September 26, 2018. There are three datasets but will only need two of them, the ratings and the movies. 
...
The ratings dataset has the most pieces of information as it has ratings and timestamps. The ratings were scaled from **0 to 5 with a .5 step**. 

Exploring the datasets has some interesting facts concerning the dataset. For one, the distribution is slightly skewed left with the **mean** rating happening to be roughly **3.5**. There are a handful of users that contribute to the reviews given which may have bias depending on who the user was. **Most movies were not rated below 3** so it might be difficult to determine a good movie to recommend.  
...
There was a **slight difference** between the Baseline version and the SVD model. **KnnBaseline** performs slightly better and was used for this project. 

## User Example 1 Recommendation
### Individual Results 
The **Top movies** for these methods are as follows for user 1:
...
The **first** model is based on the **similarities of users** and the movies they like. The idea would be that users have similar likes and dislikes and should be recommended to them. They should rate the movies similarly. <br> 
The **second** model compares the ratings from movies and **compares them to similar-rated movies**. If the movies have similar ratings from one another, a user may rate that movie similarly as well. Both of the models didn't perform well with a **.8776 and .8899 RSME score**. <br> RSME score gives an average rating that the predicted values gave from the true value. That would mean that a movie was rated on average higher by **.87 to .89** more than the true values. In the attempt to see if **combining the models**, there was a slight increase in predictions but not much. 

To better the model, some measures were placed when dealing with cold starts. **Cold starts** are situations in a given system that need some help to get started before being implemented correctly. 
...
3. There were **many ratings from the same users**. There can be a **bias** between users especially when the majority of the ratings are from them. These users could also be critics who may be very helpful to the average moviegoer. 
